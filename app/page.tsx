...
const INITIAL_DATA: LoteData = {
  ...
  lotesAdyacentes: {
    norte: {
      ...
      propietario: "Inversiones Santa Rosa S.A.C.",
      vertices: [
        { id: "A", x: 284500.0, y: 8670120.0 },
        { id: "B", x: 284510.0, y: 8670120.0 },
        { id: "C", x: 284510.0, y: 8670100.0 },
        { id: "D", x: 284500.0, y: 8670100.0 },
      ],
    },
    sur: {
      id: "ltn2",
      ...
      propietario: "Inversiones Santa Rosa S.A.C.",
      vertices: [
        { id: "A", x: 284500.0, y: 8670080.0 },
        { id: "B", x: 284510.0, y: 8670080.0 },
        { id: "C", x: 284510.0, y: 8670060.0 },
        { id: "D", x: 284500.0, y: 8670060.0 },
      ],
    },
    este: {
      id: "ltn3",
      ...
      propietario: "Inversiones Santa Rosa S.A.C.",
      vertices: [
        { id: "A", x: 284510.0, y: 8670100.0 },
        { id: "B", x: 284520.0, y: 8670100.0 },
        { id: "C", x: 284520.0, y: 8670080.0 },
        { id: "D", x: 284510.0, y: 8670080.0 },
      ],
    },
    oeste: {
      id: "ltn4",
      ...
      propietario: "Inversiones Santa Rosa S.A.C.",
      vertices: [
        { id: "A", x: 284490.0, y: 8670100.0 },
        { id: "B", x: 284500.0, y: 8670100.0 },
        { id: "C", x: 284500.0, y: 8670080.0 },
        { id: "D", x: 284490.0, y: 8670080.0 },
      ],
    },
  },
};
...
export default function EditorPlanos() {
  ...
  const [showNeighborsImportModal, setShowNeighborsImportModal] =
    useState(false);
  const [selectedNeighbor, setSelectedNeighbor] = useState<LoteVecino | null>(
    null,
  );
  ...
  const handleImportNeighbors = (newCoords: UTMCoordinate[]) => {
    ...
    setData((prev) => ({
      ...
      contexto: {
        ...prev.contexto,
        vecinos: [...prev.contexto.vecinos, newNeighbor],
      },
    }));
  };
  ...
  const handleSearchOSM = async () => {
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        toast.success(
          `Ubicación encontrada: ${results[0].display_name.split(",")[0]}`,
        );
      } else {
  ...
  const previewData = useMemo(() => {
    try {
      ...
      const viewDeltaX =
        Math.abs(viewMaxX - viewMinX) < 0.001 ? gridStep : viewMaxX - viewMinX;
      const viewDeltaY =
        Math.abs(viewMaxY - viewMinY) < 0.001 ? gridStep : viewMaxY - viewMinY;
      ...
      const technicalData = points.map((p, i) => {
        ...
        const dist = Math.sqrt(
          Math.pow(rawNext.x - rawP.x, 2) + Math.pow(rawNext.y - rawP.y, 2),
        );
        totalPerimeter += dist;

        const angleToPrev =
          Math.atan2(prevP.y - p.y, prevP.x - p.x) * (180 / Math.PI);
        const angleToNext =
          Math.atan2(nextP.y - p.y, nextP.x - p.x) * (180 / Math.PI);
        ...
        if (endArc < startArc)
          bisectorAngle = startArc + (360 - startArc + endArc) / 2;
      ...
      const adyacentes = Object.entries(data.lotesAdyacentes || {})
        .map(([key, info]) => {
          ...
          const leafletPolygon = info.vertices.map((v) =>
            utmToLatLng(v.x, v.y),
          );
      ...
      return {
        ...
        centerLabel: {
          x: centerX,
          y: centerY,
          perimeter: totalPerimeter.toFixed(2),
        },
        elements: {
          locMap: {
            x: locMapX,
  ...
  const handleSvgMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    if (draggingVertexIndex !== null && svgRef.current && previewData) {
      ...
      newVertices[draggingVertexIndex] = {
        ...newVertices[draggingVertexIndex],
        x: Number(newCoords.x.toFixed(2)),
        y: Number(newCoords.y.toFixed(2)),
      };
  ...
  const handleInputChange = (
    section: "dimension" | "lote",
    field: keyof LoteData["lotesAdyacentes"],
    value: string | number,
  ) => {
    setData((prev) => ({
      ...prev,
  ...
  const handleMembreteChange = (field: keyof MembreteData, value: string) => {
    setData((prev) => ({
      ...prev,
      membrete: { ...prev.membrete, [field]: value },
    }));
  };
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const hybridPayload = {
        ...
        datosRegistrales: {
          areaOficial: data.area, // Usar área del formulario
          ...
          linderos: data.vertices.map((v, i) => {
            ...
            let orientacion: "NORTE" | "ESTE" | "SUR" | "OESTE" = "NORTE";

            if (data.vertices.length === 4) {
              // Para polígonos rectangulares (4 lados)
              const mapping = [
                {
                  dist: data.lotesAdyacentes.norte?.dimension,
                  col: data.lotesAdyacentes.norte?.lote,
                  ori: "NORTE" as const,
                },
                {
                  dist: data.lotesAdyacentes.este?.dimension,
                  col: data.lotesAdyacentes.este?.lote,
                  ori: "ESTE" as const,
                },
                {
                  dist: data.lotesAdyacentes.sur?.dimension,
                  col: data.lotesAdyacentes.sur?.lote,
                  ori: "SUR" as const,
                },
                {
                  dist: data.lotesAdyacentes.oeste?.dimension,
                  col: data.lotesAdyacentes.oeste?.lote,
                  ori: "OESTE" as const,
                },
              ];

              if (mapping[i]) {
                longitudTexto = mapping[i].dist?.toFixed(2) || "0.00";
        ...
        // Contexto geoespacial (lotes vecinos + lotes adyacentes)
        contexto: {
          type: "FeatureCollection" as const,
          features: [
            // Vecinos del contexto
      ...
      if (dataSourceArea === "CALCULADO") {
        toast.success("📐 Área calculada automáticamente desde geometría", {
          duration: 3000,
        });
      }
        ...
        <header className="max-w-[1400px] mx-auto mb-6 flex flex-col md:flex-row justify-between items-center bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="flex gap-3 mt-4 md:mt-0">
            ...
            <button
              ...
              disabled={loading}
              className="px-6 py-2 text-sm font-medium text-white bg-blue-800 hover:bg-blue-900 rounded-lg shadow-md flex items-center gap-2 transition-transform active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {loading ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={data.loteId}
                          onChange={(e) =>
                            handleRootChange("loteId", e.target.value)
                          }
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm outline-none focus:ring-2 focus:ring-blue-800"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleRootChange("propietario", e.target.value)
                          }
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm outline-none focus:ring-2 focus:ring-blue-800"
                        />
                      </div>
                              ...
                              <input
                                ...
                                value={
                                  data.lotesAdyacentes[
                                    lado as keyof typeof data.lotesAdyacentes
                                  ]?.dimension
                                }
                                onChange={(e) =>
                                  handleInputChange(
                ...
                {activeTab === "membrete" && (
                  <div className="space-y-5 animate-fadeIn">
                    <div className="bg-yellow-50 border border-yellow-100 p-3 rounded-lg text-xs text-yellow-800 mb-4">
                      <span className="font-bold">Nota:</span> Datos para el
                      cajetín oficial.
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleMembreteChange("proyecto", e.target.value)
                        }
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleMembreteChange("plano", e.target.value)
                          }
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleMembreteChange("lamina", e.target.value)
                          }
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm text-center font-mono"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleMembreteChange("profesional", e.target.value)
                        }
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleMembreteChange("registro", e.target.value)
                          }
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleMembreteChange("fecha", e.target.value)
                          }
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleMembreteChange("escala", e.target.value)
                        }
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm"
                      >
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          >
                            ...
                            />
                            <div
                              className={`text-xs font-bold ${data.config.modoUbicacion === modo.id ? "text-indigo-900" : "text-slate-700"}`}
                            >
                              {modo.label}
                            </div>
                          </button>
                      ...
                      <div className="space-y-3">
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-3">
                          ...
                          <p className="text-[10px] text-blue-600">
                            Búsqueda gratuita powered by Nominatim /
                            OpenStreetMap.
                          </p>
                          <div className="text-xs text-slate-600 bg-white p-2 rounded border border-blue-100">
                            💡 El buscador trasladará tu lote y vecinos a la
                  ...
                  <div className="animate-fadeIn h-full flex flex-col">
                    ...
                    <div className="flex-1 space-y-2">
                      {data.vertices.map((vertice, index) => (
                        <div
                          key={index}
                          className={`flex gap-2 items-center p-2 rounded border transition-colors ${hoveredVertexId === vertice.id ? "bg-orange-50 border-orange-300 ring-1 ring-orange-200" : "bg-slate-50 border-slate-200"}`}
                          onMouseEnter={() => setHoveredVertexId(vertice.id)}
                        ...
                        >
                          <input
                            ...
                            value={vertice.id}
                            onChange={(e) =>
                              handleVertexChange(index, "id", e.target.value)
                            }
                          ...
                          <div className="flex-1 grid grid-cols-2 gap-2">
                            <div className="relative">
                              <span className="absolute left-1.5 top-1 text-[9px] font-bold text-slate-400">
                                E
                              </span>
                              <input
                                type="number"
                                value={vertice.x}
                                onChange={(e) =>
                                  handleVertexChange(index, "x", e.target.value)
                                }
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                value={vertice.y}
                                onChange={(e) =>
                                  handleVertexChange(index, "y", e.target.value)
                                }
                                className="w-full pl-4 pr-1 py-1 text-xs bg-white border border-slate-200 rounded focus:border-blue-800 focus:outline-none"
                              />
                            </div>
                          ...
                          <button
                            ...
                            disabled={data.vertices.length <= 3}
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
            ...
            <div className="bg-white p-1 rounded-xl shadow-md border border-slate-200 h-full flex flex-col">
                ...
                <div className="flex gap-4 items-center">
                  <div className="flex bg-white rounded-lg border border-slate-200 p-0.5 shadow-sm">
                    <button
                      ...
                      className="p-1.5 hover:bg-slate-50 text-slate-600 rounded-md transition-colors"
                      title="Zoom Out"
                    >
                      <ZoomOut size={16} />
                    </button>
                    <div className="px-2 flex items-center text-[10px] font-bold text-slate-500 border-x border-slate-100 min-w-[50px] justify-center">
                      {Math.round(zoom * 100)}%
                    </div>
                    <button
                    ...
                    <button
                      onClick={() => setZoom(1.0)}
                      className="p-1.5 hover:bg-slate-50 text-blue-800 rounded-md transition-colors text-[10px] font-bold"
                      title="Reset Zoom"
                    >
                      1:1
                    </button>
              ...
              <div className="flex-1 relative bg-white overflow-hidden flex items-center justify-center min-h-[600px] overflow-x-auto select-none bg-slate-100 p-4">
                {showPDFViewer ? (
                  ...
                  data.imagenGeneral ? (
                  // --- VISTA IMAGEN ---
                  <div className="w-full h-full relative overflow-hidden rounded border border-slate-300 z-0 bg-slate-200">
                    <div
                      className={`flex items-center justify-center min-w-full min-h-full transition-transform ${isDraggingImage ? "duration-0 cursor-grabbing" : "duration-200 cursor-grab"}`}
                      style={{
                    ...
                    >
                      <img
                        src={data.imagenGeneral}
                        alt="Plano General Estático"
                    ...
                    <div className="absolute bottom-6 right-6 flex flex-col gap-2 z-[1000]">
                      <button
                        ...
                        className="bg-white p-2 rounded shadow hover:bg-slate-50 transition-colors border border-slate-200"
                  ...
                  <div className="w-full h-full relative rounded border border-slate-300 overflow-hidden z-0">
                    ...
                    >
                      ...
                      {previewData.adyacentes.map((ady, idx) => (
                        <Polygon
                          key={`leaf-ady-${idx}`}
                          positions={ady.leafletPolygon as any}
                          pathOptions={{
                            color: "#94a3b8", // Dotted border color
                            dashArray: "4, 4",
                            weight: 2,
                            fillColor: "#f8fafc",
                            fillOpacity: 0.9, // Lighter shadow/fill
                          }}
                        />
                      ))}
                      <Polygon
                        positions={previewData.leaflet.polygon as any}
                        pathOptions={{
                          color: "red",
                          fillColor: "orange",
                          fillOpacity: 0.2,
                      ...
                      {typeof window !== "undefined" && (
                        <Marker
                          position={previewData.leaflet.center as any}
                          icon={getCustomIcon()}
                        />
                      )}
                    </MapContainer>
                  ...
                  >
                    ...
                    <g>
                        ...
                        {previewData.gridLines.map((line, i) => (
                          <g key={`grid-${i}`}>
                            <line
                              ...
                              stroke="#f1f5f9"
                              strokeWidth="1"
                              strokeDasharray={line.type === "v" ? "5,5" : "0"}
                            />
                            {line.type === "v" && (
                              <text
                                x={line.x1}
                                y={
                                  previewData.layout.rect2.y +
                                  previewData.layout.rect2.h -
                                  5
                                }
                                fontSize="8"
                                fill="#94a3b8"
                            ...
                            {line.type === "h" && (
                              <text
                                x={previewData.layout.rect2.x + 5}
                                y={line.y1 + 3}
                                fontSize="8"
                                fill="#94a3b8"
                              >
                      ...
                      <g
                        transform={`translate(${previewData.layout.rect2.x + previewData.layout.rect2.w - 40}, ${previewData.layout.rect2.y + 40})`}
                      ...
                      <polygon
                        points={previewData.polygonPoints}
                      ...
                      <g
                        transform={`translate(${previewData.elements.locMap.x}, ${previewData.elements.locMap.y - 20})`}
                      >
                        <rect
                          x="-50"
                          y="-12"
                        ...
                        <text
                          ...
                          fill="#000"
                        >
                          PERÍM: {previewData.centerLabel.perimeter} ml
                        </text>
                      </g>
                      ...
                      {previewData.technicalData.map((dataPoint, i) => {
                        const isHovered =
                          hoveredVertexId === dataPoint.vertex ||
                        ...
                        return (
                          <g key={`data-${i}`}>
                            ...
                            />
                            <text
                              x={dataPoint.screen.labelX}
                              y={dataPoint.screen.labelY}
                              textAnchor="middle"
                            ...
                            <g style={{ pointerEvents: "none" }}>
                              <rect
                                x={dataPoint.screen.midX - 18}
                                y={dataPoint.screen.midY - 7}
                                width="36"
                                height="14"
                                fill="white"
                                stroke="#e2e8f0"
                              ...
                              />
                              <text
                                x={dataPoint.screen.midX}
                                y={dataPoint.screen.midY + 3}
                                textAnchor="middle"
                                fontSize="9"
                                fill="#000"
                                fontWeight="bold"
                            ...
                            <g
                              ...
                              onMouseEnter={() =>
                                setHoveredVertexId(dataPoint.vertex)
                              }
                              onMouseLeave={() => setHoveredVertexId(null)}
                              onMouseDown={(e) => {
                                e.stopPropagation();
                            ...
                            >
                              <circle
                                cx={dataPoint.screen.x}
                                cy={dataPoint.screen.y}
                                r="20"
                                fill="transparent"
                              />
                              {isHovered && (
                              ...
                              <circle
                                ...
                                r="3.5"
                                fill="white"
                                stroke={isHovered ? "#ea580c" : "#000"}
                                strokeWidth={isHovered ? 2 : 1}
                              />
                              <text
                                x={dataPoint.screen.x + 6}
                                y={dataPoint.screen.y - 6}
                                fill={isHovered ? "#ea580c" : "#000"}
                    ...
                    {/* COLUMNA DERECHA */}
                    <g
                      transform={`translate(${previewData.elements.locMap.x}, ${previewData.elements.locMap.y})`}
                    >
                      <text
                        x={previewData.elements.locMap.w / 2}
                        y="-8"
                        textAnchor="middle"
                        fontSize="9"
                        fontWeight="bold"
                      ...
                      <rect
                        x="0"
                        y="0"
                        width={previewData.elements.locMap.w}
                        height={previewData.elements.locMap.h}
                        fill="white"
                        stroke="#000"
                        strokeWidth="1"
                      />
                      ...
                      <g style={{ pointerEvents: "none" }}>
                        <text
                          x={previewData.elements.locMap.w - 15}
                          y="20"
                          fontSize="10"
                          fontWeight="bold"
                        >
                        ...
                        <line
                          x1={previewData.elements.locMap.w - 11}
                          y1="22"
                          x2={previewData.elements.locMap.w - 11}
                          y2="12"
                          stroke="#000"
                          strokeWidth="1"
                        />
                        <polygon
                          points={`${previewData.elements.locMap.w - 11},12 ${previewData.elements.locMap.w - 13},15 ${previewData.elements.locMap.w - 9},15`}
                          fill="#000"
                        />
                      </g>
                    </g>
                    ...
                    {/* Tabla */}
                    <g
                      transform={`translate(${previewData.elements.table.x}, ${previewData.elements.table.y})`}
                      style={{ pointerEvents: "none" }}
                    >
                      <rect
                        x="0"
                        y="0"
                        width={previewData.elements.table.w}
                        height="20"
                        fill="#000000"
                      />
                      <text
                        x={previewData.elements.table.w / 2}
                        y="14"
                        textAnchor="middle"
                        fill="white"
                        fontSize="10"
                      ...
                      <g transform="translate(0, 20)">
                        <rect
                          ...
                          strokeWidth="0.5"
                        />
                        {[0.12, 0.25, 0.4, 0.55, 0.77].map((p, k) => (
                          <line
                            key={k}
                            x1={previewData.elements.table.w * p}
                            y1="0"
                            x2={previewData.elements.table.w * p}
                            y2="15"
                            stroke="#000"
                            strokeWidth="0.5"
                          />
                        ...
                        <text
                          x={previewData.elements.table.w * 0.06}
                          y="11"
                          textAnchor="middle"
                          fontSize="7"
                          fontWeight="bold"
                        ...
                        <text
                          x={previewData.elements.table.w * 0.185}
                          y="11"
                          textAnchor="middle"
                          fontSize="7"
                          fontWeight="bold"
                        ...
                        <text
                          x={previewData.elements.table.w * 0.325}
                          y="11"
                          textAnchor="middle"
                          fontSize="7"
                          fontWeight="bold"
                        ...
                        <text
                          x={previewData.elements.table.w * 0.475}
                          y="11"
                          textAnchor="middle"
                          fontSize="7"
                          fontWeight="bold"
                        ...
                        <text
                          x={previewData.elements.table.w * 0.66}
                          y="11"
                          textAnchor="middle"
                          fontSize="7"
                          fontWeight="bold"
                        ...
                        <text
                          x={previewData.elements.table.w * 0.885}
                          y="11"
                          textAnchor="middle"
                          fontSize="7"
                          fontWeight="bold"
                    ...
                    {/* Membrete */}
                    <g
                      transform={`translate(${previewData.elements.membrete.x}, ${previewData.elements.membrete.y})`}
                      style={{ pointerEvents: "none" }}
                    >
                      <rect
                        x="0"
                        y="0"
                        width={previewData.elements.membrete.w}
                        height={previewData.elements.membrete.h}
                        fill="white"
                        stroke="#000"
                        strokeWidth="2"
                      />
                      <line
                        x1={previewData.elements.membrete.w * 0.65}
                        y1="0"
                        x2={previewData.elements.membrete.w * 0.65}
                        y2={previewData.elements.membrete.h}
                        stroke="#000"
                        strokeWidth="1"
                      />
                      <line
                      ...
                      <line
                        ...
                        strokeWidth="1"
                      />
                      <text x="5" y="10" fontSize="7" fill="#666">
                        PROYECTO:
                      </text>
                      <text
                        x="5"
                        ...
                        fontWeight="bold"
                      >
                        {data.membrete.proyecto.substring(0, 35)}
                      </text>
                      <text x="5" y="35" fontSize="7" fill="#666">
                        PLANO:
                      </text>
                      <text
                        x="5"
                      ...
                      <text x="5" y="60" fontSize="7" fill="#666">
                        PROFESIONAL:
                      </text>
                      <text x="50" y="60" fontSize="7" fill="#000">
                        {data.membrete.profesional}
                      </text>
                      <text x="5" y="85" fontSize="7" fill="#666">
                        FECHA:
                      </text>
                      <text x="35" y="85" fontSize="7" fill="#000">
                        {data.membrete.fecha}
                      </text>
                      <text x="90" y="85" fontSize="7" fill="#666">
                        ESC:
                      </text>
                      <text x="110" y="85" fontSize="7" fill="#000">
                        {data.membrete.escala}
                      </text>
                      <text
                        x={previewData.elements.membrete.w * 0.825}
                        y="30"
                        fontSize="10"
                        fill="#666"
                        textAnchor="middle"
                  ...
                  <div className="text-slate-400 flex flex-col items-center justify-center h-full gap-2">
                    <AlertCircle
                      size={48}
                      className="mb-2 opacity-20 text-red-500"
                    />
                    <span className="font-bold text-slate-500">
                      Error de Geometría
                    </span>
                    <span className="text-xs text-slate-400">
                      Verifique que las coordenadas formen un polígono válido.
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white w-full h-full rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Vista ampliada con referencia geográfica real
                </p>
              </div>
              <button
