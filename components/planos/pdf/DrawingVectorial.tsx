...
import { View, Text, Svg, Path, Line, Circle, Rect, G } from '@react-pdf/renderer';
...
export const DrawingVectorial = ({ vertices, datosTecnicos, area, perimetro, lotesAdyacentes = [] }: Props) => {
    ...
    <View style={{ width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center' }}>
      <Svg width={VIEWPORT_W} height={VIEWPORT_H}>
        ...
        {gridLinesX.map((x, i) => {
          ...
          return (
            <Text key={`lx-${x}`} x={p.x} y={VIEWPORT_H - 5} style={{ fontSize: 5, fill: '#444' }} textAnchor="middle">
              {x.toFixed(0)}E
            </Text>
          );
        })}
        {gridLinesY.map((y, i) => {
          ...
          return (
            <Text key={`ly-${y}`} x={5} y={p.y} style={{ fontSize: 5, fill: '#444' }} textAnchor="start">
              {y.toFixed(0)}N
            </Text>
          );
        })}
        ...
        {lotesAdyacentes.map((lote, index) => {
          const pathDataAdyacente = lote.vertices.map((v, i) => {
            const p = toPaper(v.x, v.y);
            return `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`;
          }).join(' ') + ' Z';
          
          const cx = lote.vertices.reduce((acc, v) => acc + v.x, 0) / lote.vertices.length;
          const cy = lote.vertices.reduce((acc, v) => acc + v.y, 0) / lote.vertices.length;
          const centroid = toPaper(cx, cy);

          return (
            <G key={`adyacente-${index}`}>
              <Path d={pathDataAdyacente} fill="none" stroke="#999" strokeWidth={1} strokeDasharray="4,4" />
              <Text x={centroid.x} y={centroid.y} style={{ fontSize: 6, fill: '#666' }} textAnchor="middle">
                {lote.id}
              </Text>
            </G>
          );
        ...
        {vertices.map((v, i) => {
          ...
          return (
            <G key={`v-${i}`}>
              ...
              <Text x={p.x + 15} y={p.y - 10} style={{ fontSize: 5, fill: '#666' }}>
                {decimalToDMSShort(anguloVal)}
              </Text>

              {/* Vértice */}
              <Circle cx={p.x} cy={p.y} r={2} fill="white" stroke="#000" strokeWidth={0.5} />
              <Text x={p.x + 3} y={p.y - 3} style={{ fontSize: 6, fontWeight: 'bold' }}>
                {v.id}
              </Text>

              {/* Distancia Lado */}
              <Rect x={midX - 10} y={midY - 4} width={20} height={8} fill="white" />
              <Text x={midX} y={midY} style={{ fontSize: 5 }} textAnchor="middle">
                {distVal}m
              </Text>
            </G>
          );
        ...
        <G transform={`translate(${VIEWPORT_W - 50}, 50)`}>
           ...
           <Text x={-3} y={-25} style={{ fontSize: 10, fontWeight: 'bold' }}>N</Text>
        </G>
