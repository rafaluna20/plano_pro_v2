        ...
        <View style={styles.container}>
          ...
          <View style={styles.drawingArea}>
            {modoUbicacion === 'vectorial' && (
              <DrawingVectorial vertices={vertices} datosTecnicos={datosTecnicos} area={area} perimetro={perimetro} lotesAdyacentes={lotesAdyacentes} />
            )}
          ...
          <View style={styles.infoColumn}>
            ...
            <View style={[styles.sectionBox, { height: 120, alignItems: 'center', justifyContent: 'center', padding: 0 }]}>
              <Text style={[styles.sectionHeader, { width: '100%', marginBottom: 0 }]}>UBICACIÓN ESQUEMÁTICA</Text>
              <View style={{ flex: 1, width: '100%', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                {modoUbicacion === 'satelital' && satelliteUrl ? (
                  ...
                  <View style={{ width: '100%', height: '100%', backgroundColor: '#f9f9f9', padding: 5 }}>
                     <LocationSketchMini 
                        vertices={vertices} 
              ...
              <View style={{ borderTop: '0.5pt solid #000', width: '100%', paddingVertical: 2 }}>
                <Text style={{ fontSize: 5, textAlign: 'center' }}>SIN ESCALA</Text>
              </View>
            </View>
            ...
            <View style={styles.titleBlock}>
              ...
              <View style={[styles.titleBlockRow, { height: 50 }]}>
                <View style={[styles.titleBlockCell, { width: '40%', alignItems: 'center', justifyContent: 'center' }]}>
                  {logoUrl ? (
                    <Image src={logoUrl} style={{ maxWidth: '80%', maxHeight: '80%' }} />
                  ) : (
                    <Text style={{ fontSize: 12, fontWeight: 'bold' }}>LOGO</Text>
                  )}
                </View>
                <View style={[styles.titleBlockCell, { flex: 1, borderRight: 0 }]}>
 ...
 */
const LocationSketchMini = ({ vertices, lotesAdyacentes }: { vertices: Point[], lotesAdyacentes: any[] }) => {
  const W = 240;
  const H = 80;
  ...
  const scale = Math.min((W * 0.8) / (anchoReal || 1), (H * 0.8) / (altoReal || 1));
  const offsetX = (W - anchoReal * scale) / 2;
  const offsetY = (H - altoReal * scale) / 2;
  ...
  const mainPath = vertices.map((v, i) => {
  ...
  return (
    <Svg width={W} height={H}>
      {lotesAdyacentes.map((lote, idx) => {
        const d = lote.vertices.map((v: any, i: number) => {
          const p = toPaper(v.x, v.y);
          return `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`;
        }).join(' ') + ' Z';
        return <Path key={idx} d={d} fill="none" stroke="#ccc" strokeWidth={0.5} strokeDasharray="2,2" />;
      })}
      <Path d={mainPath} fill="#ddd" stroke="#000" strokeWidth={1} />
    </Svg>
  );
};
